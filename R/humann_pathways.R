...
if ( !commands[STEP][[1]]$completed ){
  ...
  # find files and send command ---------------------------------
    ...
    calls = purrr::pmap(jobs.list, function(fseq, rseq, remaining.samples){
      str_glue("cat {fseq} {rseq} > {out.dir}/{remaining.samples}.concat.fastq.gz && \\
               {humann} humann {variables.humann} --input {out.dir}/{remaining.samples}.concat.fastq.gz --output {out.dir}/{remaining.samples}")
    })
  ...
  calls = list(str_glue("{humann} humann_join_tables -i {out.dir} -o {out.dir}/merged.genefamiles.tsv \\
                        --file_name 'gene' && \\
                        {humann} humann_renorm_table --input {out.dir}/merged.genefamiles.tsv \\
                        --units 'cpm' --output {out.dir}/merged.genefamiles.cpm_norm.tsv"), 
               str_glue("{humann} humann_join_tables -i {out.dir} -o {out.dir}/merged.pathcoverage.tsv \\
                        --file_name 'coverage'"), 
               str_glue("{humann} humann_join_tables -i {out.dir} -o {out.dir}/merged.pathabundance.tsv \\
                        --file_name 'abundance' && \\
                        {humann} humann_renorm_table --input {out.dir}/merged.pathabundance.tsv \\
