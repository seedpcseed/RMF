...
if( length(ls) >0 ){
  ...
  calls <- purrr::pmap(job.list, function(fseq, rseq, kout, taxout, taxtable){
    str_glue("{kaiju} kaiju-multi {variables.kaiju.map} -i {fseq} -j {rseq} -o {kout} 2>&1 | \\
    tee -a log/classify.log && \\
    {kaiju} kaiju-addTaxonNames {variables.kaiju.tax} -i {kout} \\
    -o {taxout} 2>&1 | tee  -a log/classify.log && \\
    {kaiju} kaiju2table {variables.kaiju.table} \\
