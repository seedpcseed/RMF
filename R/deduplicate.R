...
if( length(ls) > 0 ){
  ...
  calls <- purrr::pmap(job.list, function(fseq, rseq,temp.dir, fseq.reloc, rseq.reloc, fout, rout){
    ...
    {qc} pigz -dc {fseq} > {fseq.reloc} && echo 'Decompressed {fseq.reloc}' && \\
    {qc} pigz -dc {rseq} > {rseq.reloc} && echo 'Decompressed {rseq.reloc}' && \\
    echo 'Starting deduplication' && {qc} idseq-dedup {variables.idseq.dedup} -i {fseq.reloc} -i {rseq.reloc} \\
    -o {fout} -o {rout} 2>&1 | tee -a log/deduplicate.log && echo 'Completed deduplication' && \\
    echo 'Compressing {fout} and {rout}' && {qc} pigz {fout} {rout} && \\
